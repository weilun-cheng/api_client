...
class FortiPAMApiClient(client.ApiClient):
            ...
            if key.lower() == 'set-cookie':
                ...
                if parse_cookie:
                    if parse_cookie.group('cookie_value') != '"0%260"':
                        res[parse_cookie.group('cookie_name')] = \
                            parse_cookie.group('cookie_value')
    ...
    def _login(self, conn=None, headers=None):
        ...
        g = eventlet_request.FortiPAMLoginRequestEventlet(
            self, message['method'], message['path'], message['body'], login_headers,
            client_conn=conn
        )
        g.start()
        ret = g.join()
        if ret:
            ...
            for key, value in res_headers:
                if key.lower() == 'set-cookie':
                    ...
                    cookies += f'{cookie};'
            if cookies:
                LOG.debug("Saving new authentication cookie '%s'", cookies)
        g = eventlet_request.FortiPAMLoginRequestEventlet(
            self, 'GET', '/login?redir=%2F', None, {'Cookie': cookies},
            client_conn=conn
        )
        g.start()
        ret = g.join()
    ...
    def request(self, opt, content_type=DEFAULT_CONTENT_TYPE,
        ...
        return super().request(
            opt, content_type, http_timeout, **message
        )

    def render(self, opt, formatter=const.DEFAULT_FORMATTER, **message):
        body = message.pop('body', None)
        msg = super(FortiPAMApiClient, self).render(
            opt, formatter=formatter, **message)
        if body is not None:
